...
While some genomes exist as packages on Bioconductor, for example...
...
I want to show you though, how to work with genomes that don't exist
as a package on Bioconductor. For example, the Ensembl version of the
genome doesn't exist on Bioconductor as a package, just the UCSC
versions above. The first step is to load up the *AnnotationHub*
package: 
...
Try searching for **species:** Homo sapiens, **genome:** grch38,
**description:** FASTA.
...
We can also query the AnnotationHub using the `query` function. The
genome we want is labelled `dna.primary_assembly` so we use that as a
keyword:
...
**Note!** the following line downloads the human genome sequence, which is
~700 Mb compressed to `~/.AnnotationHub`.

```{r}
genome <- query(ah, c("ensembl","GRCh38","dna.primary","release-87"))[[1]]
```

We can extract particular sequences from the genome using *GRanges*
...
# Working with the transcriptome

We can also download the cDNA, which is the sequence of the exons for
each transcript. Note that, if the gene is on the - strand, then the
cDNA will be the reverse complement of the reference genome, the
`genome` object above.
