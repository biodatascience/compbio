...
resize(gr, width=1) # just the left-most basepair
flank(gr, width=2, both=TRUE) # +/- 2 bp from one end
```

Note that the last command, `flank`, gave us intervals of length 4,
which are centered on the *start* of the ranges with positive strand,
and centered on the *end* of the ranges with negative strand. This is
particular useful for genes, in which we often care about the region
...
(Note this file is 75 Mb, so you'll want to have a decent internet
connection: it contains lots of information about all human genes.
We will later download the human genome itself, which is close to 1 Gb
compressed.)
