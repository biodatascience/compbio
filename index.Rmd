---
title: "Introduction to Computational Biology"
author: "[Michael Love](http://mikelove.github.io)"
output: 
  html_document:
    self_contained: false
---

### Course information

For official course information see Sakai (syllabus, calendar, paper PDFs, etc).
This course makes extensive use of R and assumes basic familiarity with
base R (not packages) as a prerequisite. 
A self-quiz is available 
[here](https://github.com/biodatascience/compbio/blob/gh-pages/selfquiz/selfquiz.R), 
with answers provided 
[here](https://github.com/biodatascience/compbio/raw/gh-pages/selfquiz/selfquiz_answers.rda).
You can also find a
[list of base R functions](https://github.com/biodatascience/compbio/blob/gh-pages/eda/functions.md)
that one should be familiar with.

### Schedule and course notes

For `Rmd` files, go to the
[course repo](https://github.com/biodatascience/compbio) and navigate the
directories, or best of all to clone the repo and navigate within
RStudio.


| Week | Topic | Dir. | HTML |
|------|-------|------|------|
|Aug 23         | GitHub | | [github](github.html) |
|Aug 28 & 30    | R & EDA | `eda` | [EDA](eda/EDA.html), [NAs](eda/NAs.html), [brain RNA](eda/brain_RNA.html) |
|LbrDay & Sep 6 | Bioconductor | `bioc` | [objects](bioc/objects.html), [ranges](bioc/ranges.html), [GRangesList](bioc/GRL.html) |
|Sep 11 & 13    | Bioc. continued | `bioc` | [SummarizedExperiment](bioc/SE.html), [anno](bioc/anno.html), [strings](bioc/strings.html) |
|Sep 18 & 20    | Distances & norm. | `dist` | [distances](dist/distances.html), [hclust](dist/hclust.html) |
|Sep 25 & 27    | Distances & norm. | `dist` | [batch](dist/batch.html), [sva](dist/sva.html)|
|Oct 2 & 4      | Multiple testing | | |
|Oct 9 & 11	    | *Mid-term* | | |
|Oct 16 & 18    | *Fall break* | | |
|Oct 23 & 25    | Models and EM | | |
|Oct 30 & Nov 1 | LM & Hier. models | | |
|Nov 6 & 8	    | HMM | | |
|Nov 13 & 15    | Smoothing | | |
|Nov 20 & Thxg  | *Projects* | | |
|Nov 27 & 29    | *Projects* | | |
|Dec 4          | *Projects* | | |

### Resources

* [Online R Classes and Resources](http://genomicsclass.github.io/book/pages/resources.html)
* Rafael Irizarry and Michael Love, "Data Analysis for the Life Sciences" 
[Free PDF](https://leanpub.com/dataanalysisforthelifesciences),
[HTML](http://genomicsclass.github.io/book/)
* Kasper Hansen "Bioconductor for Genomic Data Science" 
[HTML](https://kasperdanielhansen.github.io/genbioconductor/)
* Aaron Quinlan "Applied Computational Genomics" [Slides](https://github.com/quinlan-lab/applied-computational-genomics)
* Jennifer Bryan et al Stat 545 [HTML](http://stat545.com/)

---

This page was last updated on `r format(Sys.time(), "%m/%d/%Y")`. 

* [GitHub repo](https://github.com/biodatascience/compbio)
* [License](https://github.com/biodatascience/compbio/blob/gh-pages/LICENSE)
